...
def main():
        ...
        if kgsearch=="1":
            ...
            for urls in current:
                ...
                for link in soup.findAll('a', attrs={'href': re.compile("^http://kenyalaw.org/kenya_gazette/gazette/download/")}):
                    ...
                    open('KenyaGazetteLatest.pdf', 'wb').write(r.content)
    ...
    if user=="1":
        ...
        print("1. Laws of Kenya\n2. Recent Legislation")
        print("\n")

        law_ke=input("Your selection? ")
        if law_ke=="2":
            print("Fetching recent legislation...")
            recent_law = "http://kenyalaw.org/kl/index.php?id=9291"
            #If there is no such folder, the script will create one automatically
            folder_location = r'C:\Users\USER\Desktop\Biin.Online\Freebies\LegiBot\LegiBot\LegalTechProject\Recent_Laws'
            if not os.path.exists(folder_location):os.mkdir(folder_location)
            response = requests.get(recent_law)
            ...
            for link in soup.select("a[href$='.pdf']"):
                ...
                with open(filename, 'wb') as f:
                    f.write(requests.get(
                        urljoin(recent_law, link['href'])).content)
                    toaster.show_toast("Fetched complete. Check folder 'Recent_Laws'. ",
                                    " ",
                                    icon_path="scrapercon.ico",
                                    duration=5)
                    print("\n")
